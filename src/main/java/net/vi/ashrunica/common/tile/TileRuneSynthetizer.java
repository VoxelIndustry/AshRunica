...
public class TileRuneSynthetizer extends TileTickingModular implements IContainerProvider
{
    ...
    {
        return new ContainerBuilder("runesynthetizer", player).player(player).inventory().hotbar()
                .tile(this.getModule(InventoryModule.class).getInventory("basic"))
                .slotLine(0, 0, 0, 2, EnumFacing.Axis.X).create();
    }
}
